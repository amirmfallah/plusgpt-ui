...
export default function Message({
  ...
  const getError = (text) => {
    ...
    } else if(errorMessage == "no_subscription") {
      return "شما سرویس فعالی ندارید.";
    } else if(errorMessage == "usage_exceeded") {
      return "اعتبار سرویس شما تمام شده است.";
    } else {
      console.log("chello")
      return `Oops! Something went wrong. Please try again in a few moments. Here's the specific error message we encountered: ${errorMessage}`;
    }
            ...
            <div className="flex flex-grow flex-col gap-3">
              {error ? (
                  <div className="flex flex min-h-[20px] flex-grow flex-col items-start gap-2 gap-4  text-red-500">
                    <div className="rounded-md border border-red-500 bg-red-500/10 px-3 py-2 text-sm text-gray-600 dark:text-gray-100 fa">
                      {getError(text)}
                    </div>
                    {
                      text == "usage_exceeded" || text == "no_subscription"?
